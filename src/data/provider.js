...
const getPinnedDatas = (parentKey, subKey) => {
  ...
  const subCategory = parentCategory.sub.find(
...
const storePinned = (parentCategory, subCategory, emoji) => {
  return new Promise((resolve, reject) => {
    ...
    const parentCategoryIndex = pinned.findIndex(
      pin => pin.category === parentCategory
    );
    if (parentCategoryIndex === -1) {
      ...
      const subCategoryIndex = pinned[parentCategoryIndex].sub.findIndex(
        pin => pin.category === subCategory
      );
      if (subCategoryIndex === -1) {
        ...
        const emojiIndex = pinned[parentCategoryIndex].sub[
          subCategoryIndex
        ].emojis.findIndex(pin => pin.emoji === emoji.emoji);
        if (emojiIndex === -1) {
...
const getEmoji = (parentCategory, subCategory, index) => {
  const parentCategoryIndex = data.findIndex(
    pin => pin.category === parentCategory
  );
  if (parentCategory === -1) {
    return null;
  }
  const subCategoryIndex = data[parentCategoryIndex].sub.findIndex(
    pin => pin.category === subCategory
  );
  if (subCategoryIndex === -1) {
    return null;
...
const removePinned = (parentCategory, subCategory, emoji) => {
  return new Promise((resolve, reject) => {
    ...
    if (emojiIndex === 1) {
      ...
      emojiIndex ===
      pinned[parentCategoryIndex].sub[subCategoryIndex].emojis.length - 1
    ) {
      pinned[parentCategoryIndex].sub[subCategoryIndex].emojis.pop();
    } else {
      const frontArr = pinned[parentCategoryIndex].sub[
        subCategoryIndex
      ].emojis.slice(0, emojiIndex);
      const backArr = pinned[parentCategoryIndex].sub[
        subCategoryIndex
      ].emojis.slice(emojiIndex + 1);
      pinned[parentCategoryIndex].sub[subCategoryIndex].emojis = [
        ...frontArr,
        ...backArr
      ];
    }
