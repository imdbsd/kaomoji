...
class EmojiDetails extends Component {
  ...
  componentDidMount() {
    ...
    if (match.params.filter === 'pinned') {
      const pinnedEmojis = getPinnedDatas(
        match.params.parentCategory,
        match.params.subCategory
      );
      ...
      const emojiIndex = pinnedEmojis.emojis.findIndex(
        e => e.emoji === emoji.emoji
      );
      if (emojiIndex !== -1) {
        isPinned = true;
      }
          ...
          <nav className="breadcrumb" aria-label="breadcrumbs">
            <ul>
              ...
              <li className="is-active">
                <span style={{ pading: '0 .75em' }}>{match.params.index}</span>
              </li>
            </ul>
          </nav>
