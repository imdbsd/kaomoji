...
class Home extends Component {
  ...
  updateEmojis = () => {
    ...
    const { emojis } =
      this.state.filter === 'all'
        ? getDatas(parentCategory, subCategory)
        : getPinnedDatas(parentCategory, subCategory);
    if (!emojis) {
      return this.setState({
        totalPage: 0,
        emojis: []
  ...
  addToPin = async emoji => {
    const { parentCategory, subCategory } = this.getCategory();
    const { success: successPinned } = await storePinned(
      parentCategory,
      subCategory,
      emoji
    );
  ...
  removeFromPin = async emoji => {
    const { parentCategory, subCategory } = this.getCategory();
    const { success, emojis } = await removePinned(
      parentCategory,
      subCategory,
      emoji
    );
      ...
      this.setState(
        {
          ...
          totalPage,
          page:
            this.state.page > totalPage - 1 ? totalPage - 1 : this.state.page,
          emojis
        },
        () => this.closeNotif()
      );
    ...
    return (
      <Fragment>
        ...
        <section className="section">
          <Tabs filter={this.state.filter} changeFilter={this.changeFilter} />
